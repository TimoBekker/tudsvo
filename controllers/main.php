...
// Получение уникальных муниципальных районов
$district_list = [];
try {
...
<form method="get" action="">
  ...
  <label>
    ...
    <select name="company" title="Выберите компанию" onchange="this.form.submit()">
      ...
      <?php foreach ($company_list as $original): ?>
        <option value="<?php echo htmlspecialchars($original); ?>"
         ...
         <?php echo htmlspecialchars(truncate_string($original, 50)); ?>
        </option>
      <?php endforeach; ?>
    </select>
  </label>
  ...
  <!-- Остальной фильтр для инвалидов -->
  <label style="margin-left:20px;">
    Доступность для инвалидов:
    <select name="disability" title="Выберите доступность" onchange="this.form.submit()">
      <option value="">-- Без фильтрации --</option>
      <option value="Функции нижних конечностей" <?php if ($filter_disability == 'Функции нижних конечностей') echo 'selected'; ?>>
        Функции нижних конечностей
      </option>
      <option value="Функции эндокринной системы и метаболизма" <?php if ($filter_disability == 'Функции эндокринной системы и метаболизма') echo 'selected'; ?>>
        Функции эндокринной системы и метаболизма
      </option>
